...
let isRaining = true;
let rainAreaHeight = window.innerHeight / 2; // Высота области для дождя
let snowAreaHeight = window.innerHeight; // Высота области для снега
...
function RainDrops(x, y, endy, velocity, opacity) {
    ...
    this.update = function() {
        ...
        if (this.y >= rainEnd) {
            this.y = randomNum(-500, 0); //сверху дождь заново
        } else {
...
window.addEventListener('resize', () => {
    canvas.width = window.innerWidth;
    canvas.height = window.innerHeight;
    rainAreaHeight = window.innerHeight / 2; // Высота области для дождя
    snowAreaHeight = window.innerHeight; // Высота области для снега
});

// Обработчик ползунка
document.getElementById('weatherToggle').addEventListener('input', (e) => {
    console.log("Ползунок изменён на:", e.target.value); 
    isRaining = e.target.value === '0';
    console.log("isRaining:", isRaining); 
    document.getElementById('circle').style.left = (e.target.value * 100) + 'px'; // Перемещение круга
    document.getElementById('circle').style.background = isRaining ? "url('дождь.jpg') no-repeat center" : "url('ama.jpg') no-repeat center"; // Изменение фона 
});
